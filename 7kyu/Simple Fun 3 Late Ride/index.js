...
Using the bike's timer, calculate the current time. Return an answer as the sum of digits that the digital timer in the
...
function lateRide(n) {
    var hours = Math.floor(n / 60),
        minutes = n % 60,
        myTite = String(hours) + String(minutes),
        result = 0;

    while (myTite) {
        result += myTite % 10;
        myTite = Math.floor(myTite / 10);
    }
