...
function highAndLow(numbers){
    let arrNub = numbers.split(' ');

    for (var i=0; i<arrNub.length; i++) {
        arrNub[i] = Number(arrNub[i]);
    }

    let maxNub = String(Math.max.apply(null, arrNub)),
        minNub = String(Math.min.apply(null, arrNub));
